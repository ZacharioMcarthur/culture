...
class PaymentController extends Controller
{
    ...
    {
        $user = Auth::user();
        $payments = Payment::with('contenu')
            ->where('user_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        return view('front.payment-history', compact('payments'));
    }

    /**
     * Afficher la liste des paiements pour l'admin
     */
    public function index(Request $request)
    {
        $payments = Payment::with(['user', 'contenu'])
            ->orderBy('created_at', 'desc')
            ->paginate(20);
    ...
    /**
     * Afficher les détails d'un paiement pour l'admin
     */
    public function show(Payment $payment)
    {
        $payment->load(['user', 'contenu']);
    ...
    public function refund(Payment $payment)
    {
        if ($payment->status !== 'approved') {
            return redirect()->back()->with('error', 'Seuls les paiements approuvés peuvent être remboursés.');
        }

        // Logique de remboursement FedaPay à implémenter
    ...
    {
        $payments = Payment::with(['user', 'contenu'])->get();
